...
{ //选择单元格变化时的响应
    ...
    if(current.isValid())
    {
        this->LabCellPos->setText(QString::asprintf("当前单元格：%d 行，%d 列",current.row()+1,current.column()+1));
...
{ //从一个StringList 获取数据，初始化数据Model
    int rowNum=aFileContent.count(); //文本行数，第1行是标题
    theModel->setRowCount(rowNum-1); //实际数据行数
    //设置表头
...
void MainWindow::on_actInsert_triggered()
{
    ...
    theModel->insertRow(curIndex.row(),aItemList); //插入一行，需要每个Cell的Item
    theSelection->clearSelection();//清空选择项
    theSelection->setCurrentIndex(curIndex,QItemSelectionModel::Select);//设置刚插入的行为当前选择行
}
